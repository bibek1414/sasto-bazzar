...
const CompareTable: React.FC<CompareTableProps> = ({
  ...
  const { updateProductInCompare } = useSastoBazzarCompare();

  // Fetch full product details for all items to get descriptions and other fields
  const productQueries = useProductsBySlugs(
    compareItems.map((item) => item.slug!),
  );
        ...
        <tbody className="divide-y divide-gray-100">
          ...
          <tr>
            <td className="p-4 font-semibold text-gray-700 bg-gray-50/50 sticky left-0 group">
              ...
              {isLoadingDetails && (
                <Loader2
                  className="inline ml-2 animate-spin text-brand-500"
                  size={14}
                />
              )}
            </td>
