...
const Header: React.FC = () => {
  ...
  const { compareItems } = useSastoBazzarCompare();
            ...
            <div className="flex items-center justify-between gap-4 md:gap-8">
              ...
              <Link href="/" className="flex-shrink-0 flex items-center gap-2">
                {siteConfig?.logo ? (
                  ...
                  <>
                    <span className="text-2xl font-bold  text-navy-950 hidden sm:block">
              ...
              <div className="flex items-center gap-2 sm:gap-6">
                ...
                <div className="relative group">
                  ...
                  >
                    <span className="text-[10px] text-gray-500 font-medium">
                      {isAuthenticated
                        ? `Hello, ${profile?.first_name || user?.first_name || "User"}`
                        : "Hello, Sign In"}
                    </span>
                  ...
                  {isAuthenticated && (
                    <div
                      className={`absolute top-full right-0 w-56 bg-white border border-gray-100 shadow-xl rounded-2xl py-2 z-50 mt-2 transition-all duration-200 ${isAccountOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible translate-y-2"}`}
                    >
                      <Link
                        href="/profile"
                        className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-brand-600 transition-colors"
                      >
                ...
                <Link
                  href="/compare"
                  className="relative p-2 hover:bg-gray-50 rounded-full transition-colors group hidden md:flex"
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-40 bg-white md:hidden overflow-y-auto">
            ...
            <div className="p-4 space-y-4">
              <div className="font-bold text-gray-400 text-xs   mb-2">
                Categories
              </div>
              {categories.map((category) => (
                <Link
