...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const discountPercentage = product.market_price
    ? Math.round(
        ((parseFloat(product.market_price) - price) /
  ...
  return (
    <div className="group bg-white border border-gray-100 rounded-2xl hover:shadow-xl hover:border-gray-200 transition-all duration-300 flex flex-col h-full overflow-hidden relative">
      ...
      <div className="p-4 flex flex-col flex-grow bg-white border-t border-gray-50">
        <div className="text-[10px] text-gray-400   font-semibold mb-1 w-fit">
          {category}
        </div>

        <Link href={`/collections/${slug}`} className="block mb-2">
          <h3
            className="text-gray-900 font-medium text-sm leading-snug line-clamp-2 group-hover:text-brand-600 transition-colors h-10"
            title={title}
          >
            {title}
          </h3>
